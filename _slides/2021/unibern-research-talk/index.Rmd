...
--
<div class="extension-tile maximum-yellow-red"><div>In situ sensors</div></div>
--
<div class="extension-tile prussian-orange"><div>Statistics and surveys</div></div>
--
<div class="extension-tile maximum-red"><div>...and more</div></div>
</div>
...
.footnote-left[Screenshot of [ESA CCI Land cover viewer](http://maps.elie.ucl.ac.be/CCI/viewer/)]
...
.right-column[
 <div class="copyright-container">
  <img src="img/4D-mining.png" alt="Snow">
...
p <- plot_ly(ts, x = ~Time, y = ~EVI, type = 'scatter', mode = 'lines', name = "Time series") %>%
  layout(showlegend = TRUE, yaxis = list(title = "MODIS EVI"), font = list(family = 'Fira Sans'))
...
for(i in 1:9){
  v.year <- seq(from = lubridate::ymd(20040820), to = lubridate::ymd(20120930), by = '1 year')[i]
...
p
# lines_list <- list()
# year_segments <- seq(from = lubridate::ymd(20030831), to = lubridate::ymd(20120930), by = '1 year')
# for(i in 1:length(year_segments)){
#   lines_list[[i]] <- list(
#     type = "line",
#     fillcolor = line_color,
#     line = list(color = "#ff7f0e", widthh=1, dash="dot"),
#     opacity = 1,
#     xref = "Time",
#     x0 = year_segments[i],
#     x1 = year_segments[i],
...
# }
# 
# layout(p, shapes = lines_list, visible = "legendonly")

  # add_lines(name = "Time segments", x = seq(from = lubridate::ymd(20020101), to = lubridate::ymd(20120930), by = '1 year'), lubridate::ymd(20070101)), y = c(0, 1), visible="legendonly", line = list(widthh=0.5, dash="dot"))
...
.left-column[
 <div class="copyright-container">
  <img src="img/what-is-sds.png" alt="Snow">
  <div class="copyright-bottom-right">&copy; CARTO 2021</div>
</div>
]

.right-column.font-dark[

- A subset of Data Science that focuses on spatial data
...
spatial data: points, lines, polygons, rasters, coverages, geometry attributes, data cubes, reference systems, as well as higher-level concepts including how attributes relate to geometries and how this affects analysis.
...
- My research contributes to extract info form DATA EO

- I am interested in interdisciplinary applied research, and believe UBIG provides an insparing interdiscplinary environmental --- 

- With expertise and research interests I can already foresee collaborations with UBIG's groups on: 
...
Besides I also bring experience on issues in land use and land coverchanges in Brazil that can add to currently African and European contexts... 
