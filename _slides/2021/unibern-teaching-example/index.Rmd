...
<div class="extension-tile-course bg1-grey">Unit 2<br>Predictive analysis using machine learning</div>
<div class="extension-tile-course maximum-red">Unit 3<br>Detecting events and patterns</div>
</div>
...
- Basic skills in R programming language from geodata (acquired in Unit I of this course)
...
```{r ts3-mt-plot, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, cache=TRUE, results='hide', fig.show='hide'}
ts <- readr::read_csv("./img/ts3-mt.csv") 
ts <- dplyr::filter(ts, (NDVI > 0.7 & Time < lubridate::ymd(20060531)) | Time >= lubridate::ymd(20060531), Time < lubridate::ymd(20121031))
ts <- dplyr::select(ts, Time, NDVI)

plot_ly(ts, x = ~Time, y = ~NDVI, type = 'scatter', mode = 'lines', name = "Time series") %>% 
  layout(showlegend = FALSE) %>% 
  add_annotations(x = lubridate::ymd(20070420), y=0.8,
            text="Deforestation event",
            showarrow=TRUE,
            arrowhead=1,
            ax = 30,
            ay = -60) %>% 
  add_annotations(x = lubridate::ymd(20090301), y=0.45,
            text="Low vegetation (grass)",
            showarrow=TRUE,
            arrowhead=1,
            ax = -30,
            ay = -40) %>% 
  htmlwidgets::saveWidget("./img/ndvi-ts3-mt.html", selfcontained = F, libdir = "lib")
```
...
.pull-right[<iframe id="igraph" scrolling="no" style="border:none;" seamless="seamless" src="./img/ndvi-ts.html" height="400" width="100%"></iframe>]
]

<!-- <img align="left" src="./img/ndvi-ts.html" height="450"/>  -->
...
.right-column[<iframe id="igraph" scrolling="no" style="border:none;" seamless="seamless" src="./img/ndvi-ts2.html" height="400" width="100%"></iframe>]
]
...
plot_ly(ts, x = ~Time, y = ~NDVI, type = 'scatter', mode = 'lines', name = "Time series") %>% 
  add_markers() 
```
```{r ts-save-plot, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, cache=TRUE, results='hide', fig.show='hide'}
plot_ly(ts, x = ~Time, y = ~NDVI, type = 'scatter', mode = 'lines', name = "Time series") %>% 
  add_markers() %>% 
  layout(showlegend = FALSE) %>% 
  add_annotations(x = lubridate::ymd(20190109), y=0.7457, angle = 45,
            text="NDVI before deforestation event",
            showarrow=TRUE,
            arrowhead=1,
            ax = 80,
            ay = -30) %>% 
  add_annotations(x = lubridate::ymd(20190529), y=0.0897, angle = 45,
            text="NDVI after deforestation event",
            showarrow=TRUE,
            arrowhead=1,
            ax = 100,
            ay = -80) %>% 
  htmlwidgets::saveWidget("./img/ndvi-ts.html", selfcontained = F, libdir = "lib")
...
plot_ly(ts, x = ~Time, y = ~NDVI, type = 'scatter', mode = 'lines', name = "Time series") %>% 
  add_markers() %>% 
  layout(showlegend = FALSE, title = "longitude: -51.21396, latitude: -6.881071") %>% 
  htmlwidgets::saveWidget("./img/ndvi-ts2.html", selfcontained = F, libdir = "lib")
```
]
...
<br>
Slides available from [https://victor-maus.com/assets/unibern-teaching-example](https://victor-maus.com/assets/unibern-teaching-example)
